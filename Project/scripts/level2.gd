...
#instancing it
var boss_horse = preload("res://scenes/tailarcher_horse.tscn")
...
onready var lancer_spawns = get_node("lancer_spawns")
...
func spawn_booster():
	...
	if randf() < 0.001:
		...
		booster1.init(global.playerhorse_pos.x + rand_range(-500, 500))
...
func _process(delta):
	...
	if wave == 1:
		if enemyhorse_container.get_child_count() == 0:
			wave += 1
			for i in range(1):
				spawn_boss(Vector2(rand_range(600, 1000), -150))
